/************************************************************************************************
  @description       :amendQuoteSettings Component is the child component of amendSetting Component.
                      It contains form to map field values from Opportunity, Account and Quote to Amend Quote.
...
export default class AmendQuoteSetting extends LightningElement {
  ...
  connectedCallback() {
    this.fetchData().then(() => {
      // Code to execute after data is fetched
  ...
  //to get data form custom metatdta
  getQuoteFieldSetup() {
    getAmendQuoteFields()
      .then((result) => {
        if (result) {
      ...
      .catch((error) => {
        this.showToastMessage(
          "There is no Field Mapping found for AmendQuoteSettings" +
            error.message,
          "error",
          "error"
        );
  ...
  // delete function for quote tab new
